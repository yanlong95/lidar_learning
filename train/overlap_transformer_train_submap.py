    ...
    def train(self):
        ...
        for j in tqdm.tqdm(range(num_scans)):
            ...
            anchor_batch, pos_batch, neg_batch, pos_overlaps, neg_overlaps, num_pos, num_neg = (
                read_one_batch_overlaps_submap_n2n(self.img_folder, overlaps_data, anchor_submaps, pos_neg_submaps, j,
                                                   self.channels, self.height, self.width, self.num_pos_max,
                                                   self.num_neg_max, self.submap_size, self.device, shuffle=True))
