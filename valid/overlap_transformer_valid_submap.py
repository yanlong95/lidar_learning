...
from tools.fileloader import load_files, load_xyz_rot, load_overlaps, load_submaps, read_image, read_submap
from tools.read_datasets_submap import read_one_batch_overlaps_submap_n2n
...
def validation(model, top_n=5, metric='euclidean', method='overlap', dist_thresh=5.0, seq=0):
    ...
    submap_size = params['learning']['submap_size']
    ...
    valid_submaps_path = os.path.join(submaps_folder, valid_seq)
    ...
    overlaps_table = load_overlaps(valid_overlaps_table_path)
    anchor_submaps, pos_neg_submaps, kf_submaps = load_submaps(valid_submaps_path)
    ...
    with (torch.no_grad()):
        ...
        for i in tqdm.tqdm(range(num_scans)):
            ...
            anchor_batch, pos_batch, neg_batch, pos_overlaps, neg_overlaps, num_pos, num_neg = read_one_batch_overlaps_submap_n2n(
                img_folder, overlaps, anchor_submaps, pos_neg_submaps, i, channels, height, width, num_pos_max,
                num_neg_max, submap_size, device, shuffle=False)
        ...
        for i in tqdm.tqdm(range(num_scans)):
            # load a scan
            current_batch = read_submap(valid_img_folder, i, anchor_submaps[i, :], submap_size, device)
        ...
        for i in range(num_scans_kf):
            current_batch = read_submap(valid_img_kf_folder, i, kf_submaps[i, :], submap_size, device)
            current_batch = torch.cat((current_batch, current_batch), dim=0)        # no idea why, keep it now, need to test and remove !!!!!
...
if __name__ == '__main__':
    ...
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model = OverlapTransformer32Submap(height=32, width=512, channels=1, use_transformer=True).to(device)
