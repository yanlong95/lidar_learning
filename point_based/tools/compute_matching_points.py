...
class PointMatcher:
    ...
    def do_matching(self, pointcloud1, pointcloud2, pose1=None, pose2=None, num_neighbors=100, verbose=False):
        """
        ...
        if self.down_sample_:
            num_points1 = len(pc1.points)
            num_points2 = len(pc2.points)
            pc1 = self.down_sample(pc1, self.voxel_size)
            pc2 = self.down_sample(pc2, self.voxel_size)
            if verbose:
                print(f'Before down sampling, pc1 has {num_points1} points, pc2 has {num_points2} points.')
                print(f'After down sampling, pc1 has {len(pc1.points)} points, pc2 has {len(pc2.points)} points.')
