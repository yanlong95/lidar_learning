...
def read_one_batch_overlaps_submap_one2n(img_folder_path, overlaps, pos_neg_submaps, idx, channels, height, width,
                                         pos_max, neg_max, size_submap=2, device='cuda', shuffle=True):
    """
    ...
    for i in range(num_pos):
        ...
        for j in range(size_submap):
            img_path = os.path.join(img_folder_path, seq, f'{str(submaps[j]).zfill(6)}.png')
            # img_path = os.path.join(img_kf_folder_path, seq, f'png_files/512/{str(submaps[j]).zfill(6)}.png')
            img_tensor = read_image(img_path, device).squeeze()  # in shape (1, H, W)
...
def read_one_batch_overlaps_submap_n2n(img_folder_path, overlaps, anchor_submaps, pos_neg_submaps, idx, channels,
    ...
    batch = overlaps[idx, :]
    anchor_submap = anchor_submaps[idx, :]
    pos_neg_submaps = pos_neg_submaps[idx, :]
    ...
    for j in range(size_submap):
        img_path = os.path.join(img_folder_path, seq, f'{str(anchor_submap[j]).zfill(6)}.png')
    ...
    for i in range(num_pos):
        ...
        pos_samples_submap_batch[i, :, :, :width] = img_tensor
        for j in range(size_submap):
            img_path = os.path.join(img_folder_path, seq, f'{str(pos_neg_submaps[j]).zfill(6)}.png')
